...
// Inflection points (PRD Scenario B differentiator #3): the pivotal moments
// where the narrative shifted. Clicking one jumps to that point's evidence.
// Works at year (Trump) or month (COVID) granularity via lib/period.
...
export function InflectionPoints({ points, onSelectKey }: Props) {
    ...
    <div className="bg-surface-white border border-border-secondary rounded-tlds-3 p-4">
      ...
      <ol className="relative space-y-3 border-l border-border-secondary pl-4">
        {points
          ...
          .map((ip, i) => {
            const label = pointLabel(ip);
            return (
              <li key={i} className="relative">
                <span className="absolute left-[-21px] top-1 h-2.5 w-2.5 rounded-full border-2 border-surface-body bg-tl-embed-green" />
                <button
                  onClick={() => onSelectKey(pointKey(ip))}
                  className="group text-left"
                  title={`Jump to ${label}`}
                >
                  <div className="flex items-baseline gap-2">
                    <span className="font-tl-mono text-xs text-tl-embed-green group-hover:underline whitespace-nowrap">
                      {label}
                    </span>
