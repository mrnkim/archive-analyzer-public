...
export function ClipGrid({ point }: Props) {
  if (!point) {
    return (
      <div className="bg-surface-white border border-border-secondary rounded-tlds-3 p-4 text-center text-sm text-foreground-subtle">
    ...
    <div className="bg-surface-white border border-border-secondary rounded-tlds-3 p-4">
      ...
      {point.dominant_theme && (
        <div className="text-xs text-foreground-subtle mb-3">
          Theme: <span className="text-tl-embed-green">{point.dominant_theme}</span>
        </div>
      )}
    ...
    <div className="flex flex-col gap-2">
      <div className="relative">
        {scene.manifest_url ? (
          ...
          <div className="aspect-video bg-surface-secondary rounded-video-thumbnail overflow-hidden border border-border-secondary">
            <img
          ...
          <div className="aspect-video bg-surface-secondary rounded-video-thumbnail overflow-hidden border border-border-secondary flex items-center justify-center text-xs text-foreground-subtle">
            No thumbnail
          </div>
        )}
      ...
      <div className="px-0.5 text-xs space-y-1">
        <div className="text-foreground-body truncate" title={scene.title}>
          {scene.title || "(no title)"}
        </div>
        <div className="text-foreground-subtle font-tl-mono text-[11px]">
          {duration.toFixed(1)}s clip
        </div>
        {scene.reason && (
          <div
            className="text-foreground-muted leading-snug pt-0.5"
            title="Why this scene was selected"
          >
            <span className="text-tl-embed-green font-medium">Why: </span>
            {scene.reason}
          </div>
        )}
      </div>
